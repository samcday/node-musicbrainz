...
mb.browse = function (resource, linkedEntity, mbid, inc, offset, dataLimit, force, callback) {
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}

	var uri = mbBaseURI + resource + '/?' + linkedEntity + '=' + mbid;
	uri += '&offset=' + offset;
	uri += '&limit=' + dataLimit;
...
mb.search = function(resource, query, filter, force, callback){
	...
	if(query && query.length > 0 && filterStr.length > 0){
		uri += encodeURIComponent(query).replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1") + encodeURIComponent(' AND ') + filterStr;
	} else if(!query || query.length === 0){
		uri += filterStr;
	} else {
		uri += encodeURIComponent(query).replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1");
	}
...
var handleReleaseList = function(data, linkedEntities, callback) {
	var releases = [];
	...
	if ('release-list' in data && 'release' in data['release-list'] && data['release-list'].release.length > 0) {
		...
		for(var i=0; i < data.length; i++){
			...
			if (linkedEntities) {
				release.readData(linkedEntities, data[i]);
			}
...
mb.browseReleases = function (subResource, mbid, linkedEntities, force, callback) {
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}
	...
	mb.browse('release', subResource, mbid, linkedEntities, 0, 100, force, function (err, data) {
		if (err) {
			callback(err, data);
			return;
		}

		handleReleaseList(data, linkedEntities, callback);
	});
};
...
mb.searchReleases = function (query, filter, force, callback) {
	...
	mb.search('release', query, filter, force, function (err, data) {
		...
		handleReleaseList(data, undefined, callback);
	});
